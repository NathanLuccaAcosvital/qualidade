      ...
      return [
        {
          ...
          items: [
            ...
            { label: 'Service Desk', icon: Inbox, path: '/quality?view=tickets' }, // NOVO: Service Desk para Qualidade
          ]
        }
      ];
    }
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <aside className={`hidden md:flex flex-col bg-[#0f172a] text-slate-300 shadow-2xl z-[60] relative transition-all duration-500 ease-in-out ${isCollapsed ? 'w-20' : 'w-72'} overflow-visible h-screen`}>
        ...
        <nav className="flex-1 py-3 space-y-1 relative z-10 overflow-y-auto sidebar-scrollbar overflow-x-hidden min-h-0">
          {menuSections.map((section, idx) => (
            <div key={idx} className="px-3">
               ...
               {isCollapsed && section.title && idx > 0 && <div className="my-3 border-t border-slate-800/50 mx-2" />}
                 ...
                 {section.items.map((item) => {
                    ...
                    return (
                      <Link key={item.label} to={item.path} className={`group flex items-center relative py-2.5 rounded-xl transition-all duration-300 ease-out ${isCollapsed ? 'justify-center px-0 mx-1' : 'px-4 gap-3'} ${active ? 'bg-gradient-to-r from-blue-600/20 to-indigo-600/10 text-white shadow-[inset_0_1px_0_0_rgba(255,255,255,0.1)] border border-white/5 backdrop-blur-md' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent'}`}>
                        {active && !isCollapsed && <div className="absolute left-0 top-1/2 -translate-y-1/2 h-6 w-1 bg-blue-500 rounded-r-full shadow-[0_0_10px_rgba(59,130,246,0.6)]"></div>}
                        <item.icon size={isCollapsed ? 20 : 18} className={`shrink-0 transition-all duration-300 ${active ? 'text-blue-400 drop-shadow-[0_0_5px_rgba(96,165,250,0.5)]' : 'group-hover:text-white group-hover:scale-110'}`} strokeWidth={active ? 2.5 : 2}/>
        ...
        <div className="p-4 border-t border-slate-800/60 bg-[#0f172a]/30 shrink-0 relative z-10" ref={userMenuRef}>
          <div onClick={() => setIsUserMenuOpen(!isUserMenuOpen)} className={`flex items-center rounded-xl transition-all duration-300 relative ${isCollapsed ? 'justify-center p-0' : 'p-2.5 bg-slate-800/40 border border-slate-700/50 gap-3 cursor-pointer hover:border-slate-600 hover:bg-slate-800/60 group'}`}>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative bg-slate-50 w-full min-w-0">
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 h-20 bg-white/90 backdrop-blur-xl border-t border-slate-200 z-40 flex items-center justify-around pb-2 px-2 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
            {bottomNavItems.map((item, idx) => {
                const active = isActive(item.path, item.exact);
                return (
                    <Link key={idx} to={item.path} className={`flex flex-col items-center justify-center w-16 h-full space-y-1 transition-all ${active ? 'text-blue-600' : 'text-slate-400 hover:text-slate-600'}`}>
            ...
            <button onClick={() => setIsMobileMenuOpen(true)} className={`flex flex-col items-center justify-center w-16 h-full space-y-1 ${isMobileMenuOpen ? 'text-blue-600' : 'text-slate-400'}`}>
                 <div className="p-1.5 rounded-xl"><UserIcon size={24} /></div>
           ...
           <>
               ...
               <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white rounded-t-3xl z-[60] p-6 pb-24 shadow-2xl animate-in slide-in-from-bottom-full duration-300 border-t border-slate-100">
                    ...
                    <div className="flex items-center gap-4 mb-8">
                        <div className="w-14 h-14 rounded-full bg-slate-100 flex items-center justify-center text-xl font-bold text-slate-600 border border-slate-200">{user?.name.charAt(0)}</div>
                        <div>
                    ...
                    <div className="space-y-2">
                        {user?.role?.toUpperCase() === UserRole.ADMIN && (
                            <button onClick={() => { setIsN3SupportOpen(true); setIsMobileMenuOpen(false); }} className="flex items-center gap-4 px-4 py-4 w-full text-slate-600 hover:bg-slate-50 rounded-2xl border border-transparent hover:border-slate-100 transition-all font-medium">
                                <div className="p-2 bg-orange-50 text-orange-600 rounded-lg"><Server size={20} /></div> {t('admin.settings.techSupport')}
                            </button>
                        )}
                        <button onClick={() => { setIsSupportOpen(true); setIsMobileMenuOpen(false); }} className="flex items-center gap-4 px-4 py-4 w-full text-slate-600 hover:bg-slate-50 rounded-2xl border border-transparent hover:border-slate-100 transition-all font-medium">
                            <div className="p-2 bg-indigo-50 text-indigo-600 rounded-lg"><Phone size={20} /></div> {t('menu.support')}
                        </button>
                        <button onClick={logout} className="flex items-center gap-4 px-4 py-4 w-full text-red-600 hover:bg-red-50 rounded-2xl border border-transparent hover:border-red-100 transition-all font-medium">
                            <div className="p-2 bg-red-50 text-red-600 rounded-lg"><LogOut size={20} /></div> {t('common.logout')}
                        </button>
                    </div>
               </div>
           </>
