...
const ClientsToolbar = ({ searchTerm, onSearchChange, onAddClient }: any) => (
  <div className="sticky top-0 z-20 bg-white/95 backdrop-blur-md border border-slate-200 p-4 flex flex-col sm:flex-row justify-between items-center gap-4 rounded-2xl shadow-sm">
    ...
    <button 
      onClick={onAddClient} 
      className="bg-slate-900 hover:bg-slate-800 text-white px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest flex items-center gap-2 transition-all active:scale-95 shadow-lg shadow-slate-900/20"
    >
          ...
          <tr key={client.id} className="hover:bg-slate-50/80 transition-colors group">
            <td className="px-6 py-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center font-bold border border-blue-100 shadow-sm shrink-0">
